...
def find_role():
    ...
    if(user_id is not None and collection_id is not None):
        ...
        return jsonify(model_driver.get_all_user_role_collection(collection_id)),200
    elif (service_id is not None):
        return jsonify(model_driver.get_all_user_role_service(service_id)),200
    if not result:
        return '', 200
...
def delete_role(owner):
    ...
    if(user_name is not None and collection_id is not None):
        if not (model_driver.get_collection_role(owner,
                                                 collection_id).upper() == 'OWNER'):
            return error_handler(403)
            # return jsonify({'userName':user_name,'role':model_driver.get_collection_role(owner,collection_id)}),200
